...
def self_evaluate_response(llm, context: str, question: str, answer: str) -> dict:
    eval_prompt = """
    아래 답변을 평가해주세요. 정확성, 완전성, 맥락 적합성을 고려해 0~100 점수와 간단한 코멘트를 제공해주세요.
    ...
    ⚠️ 반드시 JSON 형식만 출력하고, 키 이름을 변경하지 마세요.
    항상 한국어로 평가 코멘트를 작성해주세요.
    """
    
    eval_chain = ChatPromptTemplate.from_messages([
        ("system", "당신은 Python 문서와 코드 설명에 대한 전문가 평가자입니다. 항상 한국어로 평가 코멘트를 작성하세요."),
        ("human", eval_prompt)
    ]) | llm | StrOutputParser()
...
def auto_rewrite_response(llm, context: str, question: str, answer: str) -> str:
    rewrite_prompt = """
    아래 이전 답변을 개선하여 정확하고 완전하게 작성해주세요.
    이전 답변: {answer}
    질문: {question}
    문맥: {context}
    ...
    rewrite_chain = ChatPromptTemplate.from_messages([
        ("system", "당신은 Python 문서와 코드 작성 전문가입니다. 항상 한국어로 작성해주세요."),
